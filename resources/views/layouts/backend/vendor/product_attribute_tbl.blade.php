...
@if($attr->get_vendor_product->get_vendor->user_id == auth()->user()->id)
  <tr id="id{{$attr->id}}" role="row" class="odd">
    <td class="sorting_1">{{ $attr->get_vendor_product->product_name }}</td>
    <td class="sorting_1">
    ...
    <td class="sorting_1">
      <p id="stock_n{{$attr->id}}">{{$attr->qty}}</p>
      <input id="stock_e{{$attr->id}}" type="text" style="display: none; width:70px;" value="{{$attr->qty}}">
    </td>
