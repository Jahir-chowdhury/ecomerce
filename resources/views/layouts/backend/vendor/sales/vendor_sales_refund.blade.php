            ...
            <div class="row mb-2">
                <div class="col-sm-2">
                    <div id="disableDiv" style="width: 100%;
                        ...
                        <a href="{{route('vendor.sales.history')}}" class="btn btn-primary" style="padding: 10px;">
                           ...
                           @foreach ($sales->unique('order_id') as $sale)
                                @if($sale->get_orders->status == 'refund')
                                    <tr role="row" class="odd">
                                        ...
                                        <td class="sorting_1">
                                            {{$amount}} TK
                                            <p>Profit : <span class="badge badge-warning">{{$prof}}</span> TK</p>
                                        </td>
                                        <td class="sorting_1">
...
@section('js')
    <script>
       ...
       function refundProduct(id){
            ...
            $.ajax({
                url: "{{ route('vendor.order.refunded') }}",
                type: "POST",
                data: {
                    "_token": "{{ csrf_token() }}",
                    "id": id,
